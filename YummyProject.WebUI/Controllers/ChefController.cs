...
namespace YummyProject.WebUI.Controllers
{
    public class ChefController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> CreateChef(CreateChefDto createChefDtos)
        {
            var client = _httpClientFactory.CreateClient();
            var jsonData = JsonConvert.SerializeObject(createChefDtos);
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PostAsync("https://localhost:7053/api/Chefs", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7053/api/Chefs?id=" + id);
            return RedirectToAction("ChefList");
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7053/api/Chefs/GetChef?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateChef(UpdateChefDto updateChefDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            await client.PutAsync("https://localhost:7053/api/Chefs/", stringContent);
            return RedirectToAction("ChefList");
        }
    }
}
