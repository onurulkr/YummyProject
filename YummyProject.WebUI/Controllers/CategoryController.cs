...
namespace YummyProject.WebUI.Controllers
{
    public class CategoryController : Controller
    {
        ...
        public async Task<IActionResult> CreateCategory(CreateCategoryDto createCategoryDtos)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7053/api/Categories", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7053/api/Categories?id=" + id);
            return RedirectToAction("CategoryList");
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7053/api/Categories/GetCategory?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateCategory(UpdateCategoryDto updateCategoryDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            await client.PutAsync("https://localhost:7053/api/Categories/", stringContent);
            return RedirectToAction("CategoryList");
        }
    }
}